...
import { estimate1RM, bestPerformance, ProgressChart, EntryForm, emptyEntryForm } from './TrackedMovementsBlock'
...
export default function MovementDetailView({ movement, athleteId, onClose, onSaveEntry }) {
        ...
        {tab === 'record' ? (
          <>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6, 1fr)', gap: 6, background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: 'var(--rl)', padding: 10 }}>
              {RM_LIST.map((r, i) => {
                const val = r === 1 ? oneRM : rmValues[i]
                const isMain = r === 1
                return (
                  <div key={r} style={{
            ...
            <div style={{ background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: 'var(--rl)', padding: '20px 16px', textAlign: 'center' }}>
              <div style={{ fontSize: 36, fontWeight: 800, color: 'var(--text)' }}>{oneRM != null ? `${oneRM} kg` : '—'}</div>
              <div style={{ fontSize: 11, color: 'var(--text3)', marginTop: 2 }}>
                {best?.estimated ? `1RM estimé (à partir du ${best.from}RM)` : oneRM != null ? '1RM réel' : 'Aucune donnée'}
              </div>
            </div>
            ...
            <div>
              ...
              {historyEntries.length === 0 ? (
                <div style={{ fontSize: 13, color: 'var(--text3)', fontStyle: 'italic' }}>Aucun 1RM enregistré directement — l'estimation ci-dessus se base sur tes 2-6RM.</div>
              ) : (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                  {historyEntries.map(e => (
                    <div key={e.id} style={{ display: 'flex', alignItems: 'center', gap: 10, background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: 'var(--r)', padding: '10px 12px' }}>
                      <span style={{ background: 'var(--text)', color: 'var(--bg)', fontSize: 11, fontWeight: 800, borderRadius: 6, padding: '3px 7px', flexShrink: 0 }}>1 RM</span>
                      <div style={{ flex: 1, fontSize: 13, color: 'var(--text2)' }}>{formatDateFr(e.date)}</div>
                      <div style={{ fontSize: 14, fontWeight: 800, color: 'var(--text)' }}>{e.rm1} kg</div>
                    </div>
                  ))}
                </div>
              )}
          ...
          <>
            {oneRM == null ? (
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 8, background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: 'var(--rl)', padding: 12 }}>
                {PERCENTAGES.map(p => {
                  const kg = Math.round(oneRM * (p / 100) * 2) / 2
