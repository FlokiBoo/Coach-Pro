...
export async function POST(request, { params }) {
  ...
  const { exercises, date, activityMode } = await request.json()
  ...
  const dateLabel = new Date(sessionDate + 'T00:00:00').toLocaleDateString('fr-FR', { day: 'numeric', month: 'short' })
  ...
  const { data: sess } = await supabaseAdmin.from('program_sessions')
    .insert({ program_id: prog.id, order_index: 0, title: 'Séance libre', date: sessionDate, activity_mode: activityMode === 'cardio' ? 'cardio' : 'standard' })
    .select().single()
  ...
  const toInsert = (exercises || []).filter(e => e.name?.trim()).map((e, i) => ({
    ...
    pct_low: e.pct_low !== '' && e.pct_low != null && !isNaN(parseFloat(e.pct_low)) ? parseFloat(e.pct_low) : null,
    pct_high: e.pct_high !== '' && e.pct_high != null && !isNaN(parseFloat(e.pct_high)) ? parseFloat(e.pct_high) : null,
  }))
  let insertedExos = []
  if (toInsert.length) {
