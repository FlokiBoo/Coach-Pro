...
// Permet à l'athlète de choisir lui-même ses jours d'entraînement pour un programme que le coach
// n'a pas daté séance par séance (voir programs.recommended_sessions_per_week côté coach).
export async function POST(request, { params }) {
  const { token } = await params
  const { programId, daysOfWeek } = await request.json()
  if (!programId || !Array.isArray(daysOfWeek)) return NextResponse.json({ error: 'paramètres invalides' }, { status: 400 })
  if (daysOfWeek.some(d => !Number.isInteger(d) || d < 0 || d > 6)) return NextResponse.json({ error: 'jour invalide' }, { status: 400 })
  ...
  const { error } = await supabaseAdmin.from('programs')
    .update({ athlete_days_of_week: daysOfWeek.length ? [...new Set(daysOfWeek)].sort((a, b) => a - b) : null })
    .eq('id', programId)
