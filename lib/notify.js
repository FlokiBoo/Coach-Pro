...
// Notification push (téléphone, pas email) aux athlètes que le coach vient de marquer présents à
// une séance de groupe — distincte du rappel email ci-dessus (optionnel, confirmé par le coach) :
// celle-ci part automatiquement à chaque validation de présence, sans confirmation.
export async function notifyGroupSessionAttendance({ athleteIds, sessionTitle }) {
  try {
    await fetch('/api/notify/group-session-attendance', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ athleteIds, sessionTitle }),
    })
  } catch (err) {
    console.error('notifyGroupSessionAttendance failed:', err)
  }
}
