...
// Mêmes seuils pour Passif et Actif pour l'instant (à affiner séparément plus tard si besoin).
function passifActif(t) {
  return { ok: t.ok, moderate: t.moderate }
}

function withPassifActif(entries) {
  const out = {}
  Object.entries(entries).forEach(([name, t]) => {
    out[`${name} (Passif)`] = passifActif(t)
    out[`${name} (Actif)`] = passifActif(t)
  })
  return out
}

export const JOINT_TEST_THRESHOLDS = {
  'Épaule': withPassifActif({
