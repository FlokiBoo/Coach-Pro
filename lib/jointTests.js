// Un seul mode de mesure : Actif (le sportif seul ne peut pas se faire de mobilisation passive
// fiable sans coach en face) — décision du 2026-09-06, retire le Passif.
function withActifOnly(names) {
  return names.map(n => `${n} (Actif)`)
}
...
export const JOINT_TESTS = [
  {
    joint: 'Épaule',
    tests: withActifOnly(['Rotation externe', 'Rotation interne', 'Flexion']),
  },
  {
    joint: 'Hanche',
    tests: withActifOnly(['Rotation externe', 'Rotation interne', 'Flexion (genou tendu)']),
  },
  {
    joint: 'Cheville',
    tests: withActifOnly(['Dorsiflexion']),
  },
  {
    joint: 'Colonne',
    tests: COLONNE_TESTS,
