...
// Séance de course pas encore validée dans le programme de course actif le plus ancien —
...
export async function findOrCreateRunningSession(athleteId) {
  ...
  const { data: freeProg } = await supabaseAdmin.from('programs')
    .insert({ athlete_id: athleteId, title: `Séance libre — ${dateLabel}`, coach_id: athlete?.coach_id, activity_type: 'Running 🏃‍♀️' })
    .select().single()
