...
// à la main), pour ne jamais perdre une activité même hors-programme. `referenceDate` sert au
// titre de cette séance libre — la date de l'activité elle-même (import manuel rétroactif),
// pas forcément "aujourd'hui" (cas du webhook temps réel).
export async function findOrCreateSessionForActivity(athleteId, activityTypeLabel, referenceDate = new Date()) {
  ...
  const dateLabel = referenceDate.toLocaleDateString('fr-FR', { day: 'numeric', month: 'long' })
...
// idempotent sur l'id d'activité Strava, trouve/crée la séance du bon type, upsert les chiffres
// sans écraser un ressenti déjà saisi à la main, notifie le client. Retourne { imported: bool }.
export async function processStravaActivity(athlete, activity) {
  const { data: alreadyProcessed } = await supabaseAdmin.from('program_completions')
    .select('id').eq('athlete_id', athlete.id).eq('strava_activity_id', activity.id).maybeSingle()
  if (alreadyProcessed) return { imported: false, reason: 'already_processed' }

  const activityTypeLabel = stravaActivityLabel(activity.type)
  const sessionId = await findOrCreateSessionForActivity(athlete.id, activityTypeLabel, new Date(activity.start_date))
  if (!sessionId) return { imported: false, reason: 'no_session' }
