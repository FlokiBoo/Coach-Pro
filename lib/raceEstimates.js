...
// Filtre du mode Cardio d'une séance (voir session/[sessionId]/page.js) : Run (isRunMovement)
// + Row/Ski Erg/Bike, en évitant l'ambiguïté "Row"/"Ski" — un mot aussi utilisé par des exercices
// de force (Bodyweight Row, Ring Row, A-Skip...). Les vrais mouvements/tests cardio suivent deux
// conventions de nommage dans le catalogue existant : un préfixe distance/temps ("250m Row",
// "1000m Ski", "100 Cal Assault Bike") ou "Run/Row/Ski Erg/Bike <Interval|Threshold|EF|30/30>"
// (même forme que RUN_MOVEMENT_NAMES, pas encore utilisée pour Row/Ski Erg/Bike mais prévue).
export function isCardioMovementName(name) {
  if (!name) return false
  if (isRunMovement(name)) return true
  const n = (name || '').trim()
  if (/marathon/i.test(n)) return true
  if (/\bbike\b/i.test(n)) return true
  if (/^\d[\d.,]*\s*(m|km|cal)\b/i.test(n) && /\b(run|row|ski)\b/i.test(n)) return true
  if (/^(run|row|ski erg|bike)\s+(interval|threshold|ef|30\/30)\b/i.test(n)) return true
  return false
}

// Ordre d'affichage du picker en mode Cardio : les 4 mouvements d'entraînement d'abord (EF,
// Interval, Threshold, 30/30, dans cet ordre précis), puis les tests/distances ("10km Run",
// "250m Row"...) ensuite — sinon le tri alphabétique par défaut mélange les deux (les tests
// commencent souvent par un chiffre, qui trie avant les lettres).
const CARDIO_TRAINING_SUFFIX_ORDER = ['ef', 'interval', 'threshold', '30/30']
export function cardioMovementSortKey(name) {
  const lastWord = (name || '').trim().toLowerCase().split(/\s+/).pop()
  const idx = CARDIO_TRAINING_SUFFIX_ORDER.indexOf(lastWord)
  return idx === -1 ? CARDIO_TRAINING_SUFFIX_ORDER.length : idx
}
